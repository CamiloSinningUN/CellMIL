...
class ExternalValidator:
    ...
    def __init__(self, config: EvaluationExternalValidatorConfig):
        ...
        self.use_parallelism = (
            False if self.config.final_model == FinalModel.ensemble else True
        )  # It has too many error cases currently with ensenmble
        self.max_workers = 8
        self.use_gpu = False
        self.dataloader_num_workers = 0 if self.use_parallelism else 8

        self.parallelize_ensemble_folds = (
            True  # Process all folds in parallel for ensemble
        )
    ...
    def validate(self):
        ...
        if (
            self.config.final_model == FinalModel.ensemble
            and self.config.aggregation_method == AggregationMethod.everything
        ):
            ...
            for method in methods:
                ...
                if self.df.empty:
                    ...
                    logger.info(
                        f"Successfully processed {len(self.df)} model configurations for {method}"
                    )
    ...
    def _is_model_processed(
        self, experiment_id: str, existing_results: dict[str, dict[str, Any]]
    ) -> bool:
        """
    ...
    def _process_models(self, model_dirs: list[Path]):
            ...
            try:
                ...
                if model_storage.experiment_metadata:
                    ...
                    if self._is_model_processed(experiment_name, existing_results):
                        logger.info(
                            f"Skipping already processed model: {experiment_name}"
                        )
                        continue
            except Exception as e:
        ...
        logger.info(
            f"Found {len(models_to_process)} models to process (out of {len(model_dirs)} total)"
        )
        ...
        if self.use_parallelism:
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                ...
                with tqdm(
                    total=len(models_to_process), desc="Evaluating models"
                ) as pbar:
    ...
    def _run_predictions(
        ...
        if self.config.final_model == FinalModel.final:
            if input_dim is None:
                raise ValueError(
                    "input_dim must be provided for final model predictions"
                )
            return self._predict_final(
                model_storage, dataloader, lit_model_creator, input_dim
            )
        else:
    ...
    def _predict_final(
        ...
        map_location = (
            None if torch.cuda.is_available() and self.use_gpu else torch.device("cpu")
        )
    ...
    def _predict_ensemble(
        ...
        if self.parallelize_ensemble_folds and len(fold_indices) > 1:
            ...
            logger.info(
                f"Processing {len(fold_indices)} folds in parallel for ensemble..."
            )
            ...
            logger.info(
                f"Processing {len(fold_indices)} folds sequentially for ensemble..."
            )
        ...
        with ThreadPoolExecutor(max_workers=self.ensemble_max_workers) as executor:
            ...
            for future in as_completed(future_to_fold):
                ...
                try:
                    ...
                    logger.error(
                        f"Unexpected error for fold {fold_idx}: {e}\n{traceback.format_exc()}"
                    )
    ...
    def _process_single_fold_with_trainer_creation(
        ...
        try:
            ...
            logger.error(
                f"Failed to process fold {fold_idx}: {e}\n{traceback.format_exc()}"
            )
            return None
            ...
            for batch in dataloader:
                if isinstance(dataloader, DataLoaderPyG):
                    ...
                    if len(batch) == 3:
                        ...
                        cell_types = cell_types.squeeze( # type: ignore
                            0
                        )  # [n_instances] 
    ...
    def _aggregate_fold_predictions(
        ...
        logger.info(
            f"Aggregating {len(fold_predictions)} fold predictions using method: {method}"
        )
        ...
        if is_survival:
            ...
            prob_columns = [
                col
                for col in fold_predictions[0].columns
                if col.startswith("prob_class_")
            ]

            logger.info(
                f"Classification: Found {len(prob_columns)} probability columns"
            )

            # For mean/median methods, aggregate probabilities (better than logits due to scale differences)
            if prob_columns:
                logger.info(
                    f"Aggregating {len(prob_columns)} probability columns using {method}"
                )
                ...
                for df in fold_predictions:
                    probs = cast(
                        np.ndarray[Any, Any],
                        np.stack([df[col].values for col in prob_columns], axis=1), # type: ignore
                    )  # [n_samples, n_classes] 
                ...
                logger.warning(
                    "No probability columns found, falling back to majority voting on predicted labels"
                )
    ...
    def _calculate_classification_metrics(
        ...
        if logit_columns and len(logit_columns) == n_classes:
            ...
            for i in range(n_classes):
                ...
                logit_tensors.append(
                    torch.tensor(predictions_df[col_name].values, dtype=torch.float32) # type: ignore
                )  
            ...
            logits = (
                torch.nn.functional.one_hot(y_pred, num_classes=n_classes).float()
                * 10.0
            )
