...
import cv2
import numpy as np
from scipy.ndimage import measurements # type: ignore
from scipy.ndimage.morphology import binary_fill_holes # type: ignore
...
class DetectionCellPostProcessorHV:
    def __init__(
        self,
        nr_types: int | None = None,
        magnification: int | float = 40,
        gt: bool = False,
    ) -> None:
    ...
    def post_process_cell_segmentation(
        self,
        pred_map: np.ndarray[Any, Any],
    ) -> Tuple[np.ndarray[Any, Any], dict[np.int32, dict[str, Any]]]:
