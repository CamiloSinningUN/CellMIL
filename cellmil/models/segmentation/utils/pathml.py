...
def center_crop_im_batch(
    batch: torch.Tensor, dims: tuple[int, int], batch_order: str = "BCHW"
) -> torch.Tensor:
    """
    ...
    Args:
        ...
        batch_order: Order of dimensions in the batch. Either "BCHW" or "BHWC".

    Returns:
        torch.Tensor: The center cropped batch of images
    """
    assert batch.ndim == 4, (
...
def dice_loss(
    true: torch.Tensor, logits: torch.Tensor, eps: float = 1e-3
) -> torch.Tensor:
    """
...
def get_sobel_kernels(
    size: int, dt: torch.dtype = torch.float32
    ...
    Args:
        ...
        dt: Data type for the returned tensors

    Returns:
        tuple: Horizontal and vertical Sobel kernels as tensors
    """
